...
public class FlowManager {
    ...
    internal var instructionsRootView: InstructionsRootView
    ...
    internal var currentComputedCoachMark: ComputedCoachMarkConfiguration?
    ...
    init(coachMarksViewController: CoachMarksViewController) {
        ...
        self.instructionsRootView = coachMarksViewController.instructionsRootView
    }
    ...
    internal func showNextCoachMark(hidePrevious: Bool = true) {
        ...
        if let coachMark = currentComputedCoachMark, currentIndex > 0 {
            delegate?.willHideCoachMark(with: coachMark, at: currentIndex - 1)
        }

        if hidePrevious {
            guard let coachMark = currentComputedCoachMark else { return }

            coachMarksViewController.hide(coachMark: coachMark, at: previousIndex) {
                if self.currentIndex > 0 {
                    self.delegate?.didHideCoachMark(with: coachMark, at: self.currentIndex - 1)
                }
                self.showOrStop()
            }
        } else {
    ...
    internal func showPreviousCoachMark(hidePrevious: Bool = true) {
        ...
        if let coachMark = currentComputedCoachMark {
            delegate?.willHideCoachMark(with: coachMark, at: currentIndex + 1)
        }
    ...
    internal func createAndShowCoachMark(afterResuming: Bool = false,
        ...
        if !self.isPaused {
            ...
            let computedCoachMark = currentCoachMark!.computedConfiguration(inFrame: instructionsRootView.frame)
            currentComputedCoachMark = computedCoachMark

            coachMarksViewController.show(coachMark: computedCoachMark, at: currentIndex) {
                self.canShowCoachMark = true

                self.delegate?.didShowCoachMark(
                    with: computedCoachMark,
                    after: change,
